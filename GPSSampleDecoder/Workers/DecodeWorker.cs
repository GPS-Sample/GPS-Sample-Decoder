   ...
   public class DecodeWorker
   {
		...
		private void decodeConfig(DoWorkEventArgs e, string pathToConfiguration)
      {
			...
			catch (Exception ex)
         {
            Console.WriteLine(ex.ToString());
            hasError = true;
            DecodeError("Unable to decode selcted file. \n Ensure it is an encrypted JSON file\n" +
		...
		private void decodeImages( DoWorkEventArgs e, string pathToImageList, string directoryPath )
		{
			try
			{
				StreamReader sr = new StreamReader(pathToImageList);
				...
				ImageList imageList = JsonSerializer.Deserialize<ImageList>(decryptedFile);

            foreach( var image in imageList.images)
            {
					string outpath = Path.Combine(directoryPath, image.uuid + ".JPG");
		...
		private void decodeWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
      {
         ...
         {
            DecodeCompleted(e, rawJSON, configurations);
         }
         if(hasError)
         {
            hasError = false;
